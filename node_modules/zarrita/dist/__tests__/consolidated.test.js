...
import { withConsolidatedMetadata, withMaybeConsolidatedMetadata, } from "../src/extension/consolidation.js";
...
describe("withConsolidatedMetadata", () => {
    ...
    it("throws if consolidated metadata is missing", async () => {
        ...
        let try_open = () => withConsolidatedMetadata(new FileSystemStore(root), { format: "v2" });
        await expect(try_open).rejects.toThrowError(NotFoundError);
        await expect(try_open).rejects.toThrowErrorMatchingInlineSnapshot("[NotFoundError: Not found: v2 consolidated metadata]");
    });
});
describe("withConsolidatedMetadata (v3)", () => {
    let v3root = path.join(__dirname, "../../../fixtures/v3/data.zarr/consolidated");
    ...
    it("throws if v3 consolidated metadata is missing", async () => {
        let root = path.join(__dirname, "../../../fixtures/v2/data.zarr");
        let try_open = () => withConsolidatedMetadata(new FileSystemStore(root), { format: "v3" });
        await expect(try_open).rejects.toThrowError(NotFoundError);
    });
...
describe("withConsolidatedMetadata (format array)", () => {
    it("tries formats in order", async () => {
        let root = path.join(__dirname, "../../../fixtures/v3/data.zarr/consolidated");
        let store = await withConsolidatedMetadata(new FileSystemStore(root), {
            format: ["v3", "v2"],
        });
